...
contract MoonerySale is AccessControl, TimedCrowdsale , Pausable {
  ...
  constructor (uint256 openingTime, uint256 closingTime, uint256 rate_, address payable wallet_, IERC20 token_, address capper)
    public
  ...
  function setCap(address beneficiary, uint256 cap) external {
    require(hasRole(CAPPER_ROLE, _msgSender()), "MoonerySale: caller is not capper");
  ...
  /**
    ...
    */
  function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal view override onlyWhileOpen whenNotPaused {
    super._preValidatePurchase(beneficiary, weiAmount);
    // solhint-disable-next-line max-line-length
    require(_contributions[beneficiary].add(weiAmount) <= _caps[beneficiary], "MoonerySale: beneficiary's cap exceeded");
  }
