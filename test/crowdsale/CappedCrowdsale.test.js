...
describe('CappedCrowdsale', function () {
  ...
  it('rejects a cap of zero', async function () {
    await expectRevert(CappedCrowdsaleImpl.new(rate, this.wallet, this.token.address, 0, { from: this.owner }),
  ...
  context('with crowdsale', function () {
    beforeEach(async function () {
      this.crowdsale = await CappedCrowdsaleImpl.new(rate, this.wallet, this.token.address, cap, { from: this.owner });
